...
export const renderAdjLiquidity = (pool, prices, sumLiq, caps, ownership = 1, balMultiplier, user = false) => {
	...
	if (isNaN(totalLiquidity / sumLiq * 137500)) return 0;
	const lpOwners = lpOwnership(pool);
	if (user === true) return totalLiquidity * lpOwners / sumLiq * 137500 * 52 * ownership;
	return totalLiquidity / sumLiq * 137500 * 52 * ownership;
};
...
export const renderTotalYield = (pool, prices, sumLiq, caps, balMultiplier) => {
	...
	if (totalLiquidity[1] !== 0) {
		...
		annualBAL = newTotalLiquidity(pool, prices, caps, balMultiplier)[0] / sumLiq * 137500 * 52;
	}
	if (isNaN(liquidity / sumLiq * 137500)) return 0;
